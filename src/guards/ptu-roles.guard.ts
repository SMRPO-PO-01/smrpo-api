...
export class PTURolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const roles = this.reflector.get<PROJECT_USER_ROLE[]>(
      'ptu-roles',
      context.getHandler(),
    );
    ...
    if (
      project.users.some(pu => pu.userId === user.id && roles.includes(pu.role))
    ) {
      return true;
    } else {
...
export const PTURoles = (...roles: PROJECT_USER_ROLE[]) =>
  SetMetadata('ptu-roles', roles);
